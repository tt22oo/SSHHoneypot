...
## 4. 설정 방법
**기본 설정 파일은 `configs/configs.json`에 있습니다.**
```json
{
    "configs": {
        "listen": ":22",
...
**`proc` 관련 설정은 `configs/proc`에 있습니다.**
- `cpuinfo.txt`: SSH 허니팟에서 `/proc/cpuinfo`에서 표기될 정보를 설정 할수 있습니다.
- `meminfo.txt`: SSH 허니팟에서 `/proc/meminfo`에서 표기될 정보를 설정 할수 있습니다.
- `procs.json`:
```json
{
    "1": {
...
```
- `pid`: 해당 프로세스의 pid를 설정 할수 있습니다.
- `ppid`: 부모 프로세스의 pid를 설정 할수 있습니다.
- `user`: 프로세스를 소유한 사용자를 설정 할수 있습니다.
- `cmd`: 실행된 명령어를 설정 할수 있습니다.
- `args`: 해당 프로세스를 실행할때 전달한 인자를 설정 할수 있습니다.
- `start_time`: 프로세스가 시작된 시간을 설정 할수 있습니다.
